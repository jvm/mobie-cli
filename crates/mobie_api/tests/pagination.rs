...
#[tokio::test]
async fn sync_sessions_window_continues_until_empty_page() {
    let server = MockServer::start().await;
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "data": [
                { "id": "sess-4", "start_date_time": "2025-01-04T00:00:00Z", "location_id": "EVSE-1" },
                { "id": "sess-3", "start_date_time": "2025-01-03T00:00:00Z", "location_id": "EVSE-1" }
            ],
            "status_code": 1000,
            "status_message": "Success",
            "timestamp": "2025-01-01T00:00:00Z"
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "data": [
                { "id": "sess-2", "start_date_time": "2025-01-02T00:00:00Z", "location_id": "EVSE-1" },
                { "id": "sess-1", "start_date_time": "2025-01-01T00:00:00Z", "location_id": "EVSE-1" }
            ],
            "status_code": 1000,
            "status_message": "Success",
            "timestamp": "2025-01-01T00:00:00Z"
    ...
    let mut client = authed_client(&server);
    let sessions = client
        .sync_sessions_window("EVSE-1", 2, &SessionFilters::default())
        .await
        .unwrap();

    assert_eq!(sessions.len(), 4);
    assert_eq!(sessions[0].id, "sess-1");
    assert_eq!(sessions[3].id, "sess-4");
}

#[tokio::test]
async fn sync_ocpp_logs_window_continues_until_empty_page() {
    let server = MockServer::start().await;
    ...
    assert_eq!(logs[3].timestamp.as_deref(), Some("2025-01-04T00:00:00Z"));
}
